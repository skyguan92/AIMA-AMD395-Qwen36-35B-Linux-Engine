...
The v1.5.1-native-vl.4 runtime specializes one model, one GPU architecture and
...
media, image-only batches below 1024 patches and image-only batches above 4096
patches. The WPE6 image is embedded in the executable's verified AOT registry
and is selected for image-only batches containing 1024 through 4096 patches
across one or more single-frame grids. Video and mixed requests stay on WPE3
even when their sampled grids have a temporal depth of one. Startup retains a
standard-shape WPE3 warmup for shared resources and executes the maximum
image-count warmup on WPE6 before publishing readiness.
...
`benchmarks/results/native-vl-g5-release-v1.5.1-native-vl.4.json`. The exact
