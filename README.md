...
[![Release](https://img.shields.io/badge/release-v1.5.1--native--vl.10-green.svg)](https://github.com/skyguan92/AIMA-AMD395-Qwen36-35B-Linux-Engine/releases/tag/v1.5.1-native-vl.10)
...
Version 1.5.1-native-vl.10 includes the complete fixed-model image, video, mixed-media
...
> v1.5.1-native-vl.7 reuses saved common text prefixes across divergent chats,
> including complete KV/recurrent/convolution state, bounded ownership and
> measured restoration. See the [checkpoint boundary](docs/RELEASE.md).
> v1.5.1-native-vl.9 permits verification retries after completed repairs and
> reports exhausted tool turns as explicit HTTP/SSE errors. A caller-side
> [document checker](docs/FEEDBACK_0911.md) detects text masquerading as DOCX;
> this does not establish document content quality or automatic agent completion.
> v1.5.1-native-vl.10 admits image/video tool results with validated call history
> and unchanged source policies. Attached media cannot mask explicit tool errors.
...
[native/product-contract-v1.5.1-native-vl.10.json](native/product-contract-v1.5.1-native-vl.10.json).
...
[upstream v1.5.1-native-vl.10 release](https://github.com/skyguan92/AIMA-AMD395-Qwen36-35B-Linux-Engine/releases/tag/v1.5.1-native-vl.10),
...
The v1.5.1-native-vl.10 CLI provides:
...
[patch product contract](native/product-contract-v1.5.1-native-vl.10.json) and
the hash-bound release evidence mirrored after publication. Its package-input
qualification is embedded as `share/aima/qualification.json`. The `.10`
archive must be isolated and soaked for one hour on AMD395, then rolled back to the
...
fail-closed runtime-diff rule; it is not described as an exact `.10` host run.
New text checkpoint paths require fresh cold/cache generation, full-vocabulary
logits, both capacity profiles and 19-cell text performance evidence; historical
kernel results do not qualify those new paths.
The frozen baseline and optional striped-startup evidence remain documented in
[docs/PERFORMANCE.md](docs/PERFORMANCE.md).

Run `make verify-evidence` to verify the latest mirrored summaries and every
