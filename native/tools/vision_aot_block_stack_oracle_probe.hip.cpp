...
class DeviceAllocation {
 public:
  explicit DeviceAllocation(std::size_t bytes) {
    check_hip(hipMalloc(&pointer_, bytes), "hipMalloc AOT block stack tensor");
  }
  ~DeviceAllocation() {
    if (pointer_ != nullptr) {
...
void write_file(const std::filesystem::path& path,
  ...
  if (!stream ||
    ...
    throw std::runtime_error("output capture write failed: " + path.string());
  }
}
...
int main(int argc, char** argv) {
  if (argc != 12) {
    std::cerr << "usage: native-vision-aot-block-stack-probe MODEL_DIR "
                 "AOT_IMAGE LAST_BLOCK_INDEX BLOCK0_INPUT COS SIN "
                 "EXPECTED_OUTPUT CU_SEQLENS PATCHES LOAD_REPORT "
                 "ACTUAL_OUTPUT\n";
    return 2;
  }
  try {
    ...
    const std::size_t last_block_index = std::stoull(argv[3]);
    if (last_block_index >= 27) {
      throw std::invalid_argument("last block index is outside the vision stack");
    }
    const std::size_t patches = std::stoull(argv[9]);
    if (patches == 0 ||
    ...
    const std::vector<std::uint32_t> cu_seqlens =
        parse_cu_seqlens(argv[8], patches);
    ...
    aima::NativeVisionAotBlockStackPlan plan(
        weights, std::filesystem::absolute(argv[2]), patches, cu_seqlens);
    if (plan.block_count() != 27) {
      throw std::runtime_error("native AOT vision block stack is incomplete");
    }
    ...
    upload_file(input.get(), std::filesystem::absolute(argv[4]), hidden_bytes,
                "hipMemcpy AOT block stack input");
    upload_file(cosine.get(), std::filesystem::absolute(argv[5]),
                cos_sin_bytes, "hipMemcpy AOT block stack cosine");
    upload_file(sine.get(), std::filesystem::absolute(argv[6]), cos_sin_bytes,
                "hipMemcpy AOT block stack sine");

    plan.launch_through(last_block_index, input.get(), cosine.get(), sine.get(),
                        output.get(), temporary.get(), plan.temporary_bytes());
    check_hip(hipDeviceSynchronize(),
              "hipDeviceSynchronize AOT block stack warmup");
    std::vector<unsigned char> first_output(hidden_bytes);
    check_hip(hipMemcpy(first_output.data(), output.get(), hidden_bytes,
                        hipMemcpyDeviceToHost),
              "hipMemcpy first AOT block stack output");
    ...
    for (std::size_t repetition = 0; repetition < 5; ++repetition) {
      check_hip(hipEventRecord(start), "hipEventRecord AOT block stack start");
      plan.launch_through(last_block_index, input.get(), cosine.get(), sine.get(),
                          output.get(), temporary.get(),
                          plan.temporary_bytes());
      check_hip(hipEventRecord(stop), "hipEventRecord AOT block stack stop");
      check_hip(hipEventSynchronize(stop),
                "hipEventSynchronize AOT block stack stop");
      float milliseconds = 0.0f;
      check_hip(hipEventElapsedTime(&milliseconds, start, stop),
                "hipEventElapsedTime AOT block stack");
    ...
    check_hip(hipMemcpy(actual.data(), output.get(), hidden_bytes,
                        hipMemcpyDeviceToHost),
              "hipMemcpy AOT block stack output");
    write_file(std::filesystem::absolute(argv[11]), actual);
    const Comparison comparison =
        compare_bf16(actual, std::filesystem::absolute(argv[7]));
    ...
    std::cout << std::setprecision(17)
              << "{\"schema\":\"aima-amd395-qwen36/"
                 "native-vision-aot-block-stack-oracle/v1\","
              ...
              << ",\"aot_image_sha256\":\""
              << plan.attention_image_sha256() << "\",\"measured_ms\":[";
    ...
    std::cerr << "native vision AOT block stack probe: " << error.what()
              << '\n';
    return 1;
  }
}
