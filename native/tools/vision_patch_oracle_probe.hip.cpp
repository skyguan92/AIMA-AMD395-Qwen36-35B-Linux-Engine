...
Comparison compare_bf16(const void* actual_device, std::size_t bytes,
  ...
  for (std::size_t index = 0; index < result.elements; ++index) {
    ...
    std::memcpy(&actual_bits,
                actual.data() + index * sizeof(actual_bits),
                sizeof(actual_bits));
...
int main(int argc, char** argv) {
  if (argc != 6) {
    std::cerr << "usage: native-vision-patch-probe MODEL_DIR PIXEL_ORACLE "
                 "PATCH_ORACLE PATCHES LOAD_REPORT\n";
    return 2;
  }
  void* input = nullptr;
  void* output = nullptr;
  try {
    ...
    const std::size_t input_bytes =
        patches * kPatchFeatures * sizeof(std::uint16_t);
    ...
    check_hip(hipMalloc(&input, input_bytes), "hipMalloc patch input");
    check_hip(hipMalloc(&output, output_bytes), "hipMalloc patch output");
    ...
    check_hip(hipMemcpy(input, input_host.data(), input_bytes,
              ...
              "hipMemcpy patch input");

    aima::NativeVisionPatchEmbedPlan plan(weights, patches);
    plan.launch(input, output);
    check_hip(hipDeviceSynchronize(), "hipDeviceSynchronize patch embed");
    const Comparison comparison = compare_bf16(
        output, output_bytes, std::filesystem::absolute(argv[3]));
    ...
    std::cout << std::setprecision(17)
              ...
              << "\"}\n";
    check_hip(hipFree(output), "hipFree patch output");
    output = nullptr;
    check_hip(hipFree(input), "hipFree patch input");
    input = nullptr;
    return passed ? 0 : 3;
  } catch (const std::exception& error) {
    if (output != nullptr) (void)hipFree(output);
    if (input != nullptr) (void)hipFree(input);
    std::cerr << "native vision patch probe: " << error.what() << '\n';
    return 1;
  }
}
