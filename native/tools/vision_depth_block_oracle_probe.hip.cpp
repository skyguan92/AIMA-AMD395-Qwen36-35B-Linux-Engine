...
class DeviceAllocation {
 public:
  explicit DeviceAllocation(std::size_t bytes) {
    check_hip(hipMalloc(&pointer_, bytes), "hipMalloc depth block tensor");
  }
  ~DeviceAllocation() {
    if (pointer_ != nullptr) {
...
int main(int argc, char** argv) {
  if (argc != 10) {
    std::cerr << "usage: native-vision-depth-block-probe MODEL_DIR "
                 "BLOCK_INDEX BLOCK_INPUT COS SIN BLOCK_OUTPUT CU_SEQLENS "
                 "PATCHES LOAD_REPORT\n";
    return 2;
  }
  try {
    ...
    if (block_index >= kVisionBlockCount) {
      throw std::invalid_argument("block index is outside the vision stack");
    }
    ...
    aima::NativeVisionBlockPlan plan(weights, block_index, patches, cu_seqlens);
    if (plan.block_index() != block_index) {
      throw std::runtime_error("native block plan selected the wrong depth");
    }
    ...
    for (std::size_t repetition = 0; repetition < 7; ++repetition) {
      check_hip(hipEventRecord(start), "hipEventRecord depth block start");
      plan.launch(input.get(), cosine.get(), sine.get(), output.get(),
                  temporary.get(), plan.temporary_bytes());
      check_hip(hipEventRecord(stop), "hipEventRecord depth block stop");
      check_hip(hipEventSynchronize(stop),
                "hipEventSynchronize depth block stop");
      float milliseconds = 0.0f;
      check_hip(hipEventElapsedTime(&milliseconds, start, stop),
                "hipEventElapsedTime depth block");
    ...
    std::cout << std::setprecision(17)
              << "{\"schema\":\"aima-amd395-qwen36/"
                 "native-vision-depth-block-oracle/v1\","
              << "\"complete\":" << (passed ? "true" : "false") << ','
              << "\"block_index\":" << block_index << ",\"patches\":"
              << patches << ",\"cu_seqlens\":[";
    ...
    std::cout << "],\"median_ms\":" << median_ms
              ...
              << ",\"cosine_similarity\":"
              << comparison.cosine_similarity << ",\"expected_sha256\":\""
              << comparison.expected_sha256 << "\",\"actual_sha256\":\""
    ...
    std::cerr << "native vision depth block probe: " << error.what() << '\n';
    return 1;
  }
}
