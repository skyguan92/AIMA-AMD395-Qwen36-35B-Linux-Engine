...
class DeviceAllocation {
 public:
  explicit DeviceAllocation(std::size_t bytes) {
    check_hip(hipMalloc(&pointer_, bytes), "hipMalloc block stack tensor");
  }
  ~DeviceAllocation() {
    if (pointer_ != nullptr) {
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (plan.block_count() != 27) {
      throw std::runtime_error("native vision block stack is incomplete");
    }
    ...
    check_hip(hipMemcpy(first_output.data(), output.get(), hidden_bytes,
              ...
              "hipMemcpy first block stack output");
    ...
    for (std::size_t repetition = 0; repetition < 5; ++repetition) {
      ...
      check_hip(hipEventElapsedTime(&milliseconds, start, stop),
                "hipEventElapsedTime block stack");
    ...
    std::cerr << "native vision block stack probe: " << error.what() << '\n';
    return 1;
  }
}
