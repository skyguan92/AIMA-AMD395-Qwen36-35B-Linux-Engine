...
check-cpu:
	...
	bash -n scripts/run-native-vl-performance-candidate.sh scripts/run-vl-performance-diagnostic-pair.sh scripts/run-vl-performance-matrix-pair.sh scripts/run-vllm-vl-performance-reference.sh
	...
	python3 -m py_compile scripts/aima_vllm_vl_performance_server.py scripts/capture-vl-performance-request.py scripts/generate-native-vision-pipeline-qualification.py scripts/generate-native-vl-g2-qualification.py scripts/generate-native-vl-g3-qualification.py scripts/generate-vl-performance-matrix.py scripts/qualify-vl-performance-reference-availability.py scripts/sanitize-vl-performance-reference-log.py scripts/summarize-vl-performance-diagnostic.py scripts/summarize-vl-performance-matrix-pair.py scripts/summarize-vl-performance-matrix.py scripts/summarize-vl-performance-pairs.py scripts/vllm_vl_benchmark_middleware.py
...
check-native-syntax:
	...
	g++ -std=c++17 -D__HIP_PLATFORM_AMD__ -DU_STATIC_IMPLEMENTATION -I /opt/rocm/include -I native/include -I native/generated $$(pkg-config --cflags libcurl) -fsyntax-only native/src/main.cpp native/src/decode_schedule_probe.cpp native/src/sha256.cpp native/src/native_tokenizer.cpp native/src/native_chat_protocol.cpp native/src/native_media.cpp native/src/native_remote_media.cpp native/src/native_multimodal_cache.cpp native/src/native_vl_processor.cpp native/src/native_vl_request.cpp native/src/native_vl_embedding.cpp native/src/native_mrope.cpp native/src/native_image_decoder.cpp native/src/native_video_decoder.cpp native/src/native_doctor.cpp native/src/native_http_server.cpp
