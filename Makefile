...
check-cpu: check-native-http-support
	...
	bash -n scripts/run-native-vl-performance-candidate.sh scripts/run-vl-decode-control.sh scripts/run-vl-performance-diagnostic-pair.sh scripts/run-vl-performance-matrix-pair.sh scripts/run-vl-text-decode-control.sh scripts/run-vllm-vl-performance-reference.sh
	python3 -m py_compile scripts/capture-native-lm-head-reference.py scripts/capture-native-visual-layout.py scripts/capture-vl-reference-manifest.py scripts/capture-vllm-vision-block-oracles.py scripts/capture-vllm-vision-depth-oracles.py scripts/capture-vllm-vision-multimedia-block-oracles.py scripts/capture-vllm-vision-position-oracles.py scripts/capture-vllm-vl-api-render.py scripts/capture-vllm-vl-error-limits.py scripts/capture-vllm-vl-g1-extension.py scripts/capture-vllm-vl-generation-layer-oracles.py scripts/capture-vllm-vl-generation-oracles.py scripts/capture-vllm-vl-media-io-oracles.py scripts/capture-vllm-vl-prefill-state-oracles.py scripts/capture-vllm-vl-task-quality-divergence.py scripts/capture-vllm-vl-task-quality.py scripts/capture-vllm-vl-transport-cache.py scripts/capture-vllm-vl-language-layer0-diagnostics.py scripts/capture-vllm-vl-language-layer3-mrope-diagnostics.py scripts/capture-vllm-vl-language-prefix-diagnostics.py scripts/capture-vllm-vl-oracles.py scripts/check-native-wvsplitk-parity.py scripts/check-public-tree.py scripts/diagnose-native-paired-text-tails.py scripts/export-native-aot-closure.py scripts/generate-native-aot-registry.py scripts/generate-native-decode-registry.py scripts/generate-native-decode-schedule.py scripts/generate-native-product-result.py scripts/generate-native-visual-layout.py scripts/generate-vl-capability-envelope.py scripts/generate-vl-capability-fixtures.py scripts/generate-vl-envelope-fixtures.py scripts/generate-vl-error-fixtures.py scripts/generate-vl-reference-launch.py scripts/generate-vl-task-quality-fixtures.py scripts/native_bundle_closure.py scripts/native_text_metrics.py scripts/package-release-evidence.py scripts/probe-vllm-vl-api-capabilities.py scripts/probe-vl-processor-capabilities.py scripts/qualify-native-correctness.py scripts/qualify-native-eval.py scripts/qualify-native-full-matrix.py scripts/qualify-native-paired-text-matrix.py scripts/qualify-native-openai-features.py scripts/qualify-native-portable-bundle.py scripts/qualify-native-surfaces.py scripts/qualify-native-vl-capabilities.py scripts/qualify-native-vl-envelope.py scripts/qualify-native-vl-error-limits.py scripts/qualify-native-vl-g1-extension.py scripts/qualify-native-vl-generation.py scripts/qualify-native-vl-task-quality.py scripts/qualify-native-vl-transport-cache.py scripts/trace-vllm-packed-linear-decode-aot.py scripts/trace-vllm-vision-attention-aot.py scripts/verify-native-package-inputs.py scripts/verify-release-evidence.py scripts/native_aot_trace/sitecustomize.py
	python3 -m py_compile scripts/aima_vllm_vl_performance_server.py scripts/capture-vl-performance-request.py scripts/capture-vl-text-decode-control.py scripts/generate-native-vision-pipeline-qualification.py scripts/generate-native-vl-g2-qualification.py scripts/generate-native-vl-g3-qualification.py scripts/generate-vl-performance-matrix.py scripts/qualify-vl-performance-reference-availability.py scripts/sanitize-vl-performance-reference-log.py scripts/summarize-vl-performance-diagnostic.py scripts/summarize-vl-performance-matrix-pair.py scripts/summarize-vl-performance-matrix.py scripts/summarize-vl-performance-pairs.py scripts/summarize-vl-text-decode-controls.py scripts/vllm_vl_benchmark_middleware.py
	python3 -m py_compile scripts/generate-native-bundle-manifest.py scripts/generate-native-vl-g5-qualification.py scripts/generate-native-vl-patch-g5-qualification.py scripts/generate-native-vl-patch-product-qualification.py scripts/generate-native-vl-patch-release-provenance.py scripts/generate-native-vl-product-qualification.py scripts/generate-native-vl-release-provenance.py scripts/qualify-native-chat-protocol.py scripts/qualify-native-http-control-plane.py scripts/qualify-native-temperature-sampling.py scripts/qualify-native-vl-release-gates.py scripts/qualify-native-vl-resident-soak.py scripts/qualify-native-vl-rollback.py
...
check-native-syntax: check-native-http-support
	...
	g++ -std=c++17 -Wall -Wextra -Wpedantic -Werror -O2 -pthread -I native/include tests/native_vl_processor_test.cpp native/src/native_vl_processor.cpp native/src/sha256.cpp -o build/native_vl_processor_test
	...
	g++ -std=c++17 -Wall -Wextra -Wpedantic -Werror -O2 -I native/include tests/native_mrope_test.cpp native/src/native_mrope.cpp -o build/native_mrope_test
	./build/native_mrope_test
	...
	g++ -std=c++17 -D__HIP_PLATFORM_AMD__ -DU_STATIC_IMPLEMENTATION -I /opt/rocm/include -I native/include -I native/generated $$(pkg-config --cflags libcurl) -fsyntax-only native/src/main.cpp native/src/decode_schedule_probe.cpp native/src/sha256.cpp native/src/native_tokenizer.cpp native/src/native_sampling.cpp native/src/native_chat_protocol.cpp native/src/native_media.cpp native/src/native_remote_media.cpp native/src/native_multimodal_cache.cpp native/src/native_vl_processor.cpp native/src/native_vl_request.cpp native/src/native_vl_embedding.cpp native/src/native_mrope.cpp native/src/native_image_decoder.cpp native/src/native_video_decoder.cpp native/src/native_doctor.cpp native/src/native_http_support.cpp native/src/native_http_server.cpp
...
build-native-vl-mrope-probe:
	bash scripts/build-native-vl-mrope-probe.sh

build-native-vl-language-layer0-probe:
	bash scripts/build-native-vl-language-layer0-probe.sh
...
build-native-vision-aot-attention-probe:
	bash scripts/build-native-vision-aot-attention-probe.sh

build-native-vision-aot-block-stack-probe:
	bash scripts/build-native-vision-aot-block-stack-probe.sh

build-native-vision-exact-layer-norm-probe:
	bash scripts/build-native-vision-exact-layer-norm-probe.sh
...
build-native-vision-block-probe:
	bash scripts/build-native-vision-block-probe.sh

build-native-vision-block-stack-probe:
	bash scripts/build-native-vision-block-stack-probe.sh

build-native-vision-depth-block-probe:
	bash scripts/build-native-vision-depth-block-probe.sh

build-native-vision-rotary-probe:
	bash scripts/build-native-vision-rotary-probe.sh

build-native-vision-segmented-attention-probe:
	bash scripts/build-native-vision-segmented-attention-probe.sh
