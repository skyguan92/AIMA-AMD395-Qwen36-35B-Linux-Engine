...
## PyTorch ROCm vision LayerNorm

`native/src/native_vision_exact_layer_norm.hip.cpp` reproduces the vectorized
BF16 LayerNorm reduction and expression order from
`aten/src/ATen/native/cuda/layer_norm_kernel.cu` at PyTorch commit
`8514f05131610dab50233027b2fab9c01235081b`. The native implementation removes
Torch/c10 dispatch and allocation while retaining the pinned vec4 Welford
algorithm needed for exact vision-boundary parity. The upstream BSD license is
preserved in `third_party/licenses/PYTORCH_BSD.txt`.

## vLLM/Triton vision attention image

`native/aot/gfx1151/vision-attention-v0.3.0/` contains one stripped gfx1151
code object generated by Triton 3.6.0 from vLLM's fixed vision-attention path.
Its manifest binds the source paths, Apache-2.0 and MIT provenance, compiler,
launch ABI, three-waves-per-EU scheduling, audit artifact hashes and exact
code-object transformation. The
native product loads the image directly and does not include or run the
Python, vLLM or Triton stacks.
