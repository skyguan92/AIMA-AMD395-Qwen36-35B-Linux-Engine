...
class NativeVlLanguageLayer0Test(unittest.TestCase):
    def test_logical_projection_owner_is_resident_and_vl_scoped(self) -> None:
        ...
        self.assertIn("Bf16GemmPlan& linear_output_plan()", header)
        ...
        self.assertIn("tokens, kHidden, kLinearValue", source)

        self.assertIn("logical_ab_gemm_plan->launch", linear)
        self.assertIn("launch_extract_compact_linear_ab", linear)
        self.assertIn("options.logical_output_gemm_plan", linear)
        self.assertIn("hipMemset padded logical linear-attention output", linear)
        ...
        self.assertIn("attention_options.logical_output_gemm_plan", resident)
