#!/usr/bin/env bash
# Start the frozen VL reference with an additive, hash-bound loopback test CA.
...
TLS_CA_BUNDLE="${AIMA_VL_TLS_CA_BUNDLE:?set AIMA_VL_TLS_CA_BUNDLE}"
...
if [[ ! -f "${TLS_CA_BUNDLE}" ]]; then
  echo "reference test CA bundle is missing: ${TLS_CA_BUNDLE}" >&2
  exit 1
fi
...
mkdir -p "${RUN_DIR}"
LOG_PATH="${RUN_DIR}/vllm-vl-transport-cache.log"
PID_PATH="${RUN_DIR}/vllm-vl-transport-cache.pid"
...
nohup env -i \
  ...
  REQUESTS_CA_BUNDLE="${TLS_CA_BUNDLE}" \
  SSL_CERT_FILE="${TLS_CA_BUNDLE}" \
...
server_pid=$!
printf '%s\n' "${server_pid}" >"${PID_PATH}"
printf 'transport/cache reference started: pid=%s port=%s log=%s\n' \
