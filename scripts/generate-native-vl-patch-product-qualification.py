...
TOOL_MEDIA_PREDECESSOR = "29f67beda7199575c62020d2b95e24c0a754c9a9"
TOOL_MEDIA_RUNTIME_DELTA = {"native/src/native_chat_protocol.cpp"}
TOOL_MEDIA_CHECKS = {
    "tool_media_" + name for name in (
        "image_file", "image_data", "file_data_identity", "video", "mixed",
        "replacement", "replay", "replay_identity", "parallel", "thinking",
        "default_thinking", "default_thinking_identity",
        "invalid_history_and_media_rejected", "failure_does_not_reopen_retry",
    )
}
...
def require_chat_protocol(payload: Mapping[str, Any]) -> None:
    ...
    if RELEASE in {"1.5.1-native-vl.6", "1.5.1-native-vl.7", "1.5.1-native-vl.8", "1.5.1-native-vl.9", "1.5.1-native-vl.10"} and payload["checks"].get(
        "vl_default_thinking_stream_nonstream_parity"
    ) is not True:
        raise ValueError("default VL thinking qualification is missing or failed")
    if RELEASE in {"1.5.1-native-vl.8", "1.5.1-native-vl.9", "1.5.1-native-vl.10"} and not all(
    ...
    if RELEASE == "1.5.1-native-vl.10" and not all(
        payload["checks"].get(name) is True for name in TOOL_MEDIA_CHECKS
    ):
        raise ValueError("tool media qualification is missing or failed")
...
def release_source_checks(release_commit: str) -> dict[str, bool]:
    if RELEASE == "1.5.1-native-vl.10":
        media_delta = set(git(
            "diff", "--name-only", f"{TOOL_MEDIA_PREDECESSOR}..{NATIVE_SOURCE_COMMIT}",
            "--", *RUNTIME_PATHS,
        ).stdout.splitlines())
        if media_delta != TOOL_MEDIA_RUNTIME_DELTA:
            raise ValueError("tool media patch changes the frozen .9 runtime outside chat protocol")
    if RELEASE in {"1.5.1-native-vl.8", "1.5.1-native-vl.9", "1.5.1-native-vl.10"}:
    ...
    *,
    ...
    return {
        ...
        "qualification_scope": (
            ...
            if RELEASE in {"1.5.1-native-vl.7", "1.5.1-native-vl.8", "1.5.1-native-vl.9", "1.5.1-native-vl.10"} else
...
def configure_release(contract_path: Path) -> None:
    ...
    if release not in {"1.5.1-native-vl.5", "1.5.1-native-vl.6", "1.5.1-native-vl.7", "1.5.1-native-vl.8", "1.5.1-native-vl.9", "1.5.1-native-vl.10"}:
        raise ValueError("unsupported native VL patch release")
    ALLOWED_RUNTIME_DELTA = set(SAFE_PREFIX_RUNTIME_DELTA if release in {"1.5.1-native-vl.7", "1.5.1-native-vl.8", "1.5.1-native-vl.9", "1.5.1-native-vl.10"} else CPU_RUNTIME_DELTA)
    ...
    if release == "1.5.1-native-vl.10":
        predecessor = contract.get("patch_scope", {}).get("tool_media_predecessor", {})
        if (predecessor.get("native_source_commit") != TOOL_MEDIA_PREDECESSOR
                or set(predecessor.get("allowed_runtime_paths", [])) != TOOL_MEDIA_RUNTIME_DELTA):
            raise ValueError("tool media patch changes its frozen .9 inheritance boundary")
    RELEASE = release
