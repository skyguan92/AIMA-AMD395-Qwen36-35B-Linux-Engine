#!/usr/bin/env bash
# Start the fixed VL-enabled vLLM only for capability discovery.
# This process is not an oracle until a complete reference manifest binds it.
...
if [[ ! -d "${MEDIA_ROOT}" ]]; then
  echo "capability media root is missing: ${MEDIA_ROOT}" >&2
  exit 1
fi
if [[ ! "${PORT}" =~ ^[0-9]+$ ]] || (( PORT < 1024 || PORT > 65535 )); then
  echo "invalid capability probe port: ${PORT}" >&2
  exit 1
fi
if ss -ltn | awk '{print $4}' | grep -Eq ":${PORT}$"; then
  echo "capability probe port is already in use: ${PORT}" >&2
  exit 1
fi

mkdir -p "${RUN_DIR}"
LOG_PATH="${RUN_DIR}/vllm-vl-capability.log"
PID_PATH="${RUN_DIR}/vllm-vl-capability.pid"
...
server_pid=$!
printf '%s\n' "${server_pid}" >"${PID_PATH}"
printf 'capability probe started: pid=%s port=%s log=%s\n' \
