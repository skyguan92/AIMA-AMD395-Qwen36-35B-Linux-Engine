"""Direct-seq D256 online attention with a persistent static-stride tile queue."""
...
SUPPORTED_KV_TOKENS = tuple(range(24576, 253953, 8192))


@triton.jit
def _persistent_tilequeue_d256_fwd(
    q_ptr,
    k_ptr,
    v_ptr,
    out_ptr,
    ...
    for tile_id in tl.range(
        ...
        for start_n in tl.range(0, SEQLEN_K, BLOCK_N, num_stages=1):
            ...
            qk = tl.where(key_row[None, :] <= causal_limit[:, None], qk, float("-inf"))
...
def persistent_tilequeue_d256_attention(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    out: torch.Tensor,
) -> torch.Tensor:
    """Launch the fixed persistent static-stride tile-queue kernel."""
    ...
    if k.shape[2:] != (KV_HEADS, HEAD_DIM):
        raise ValueError(f"direct seq BSHD K/V required: {tuple(k.shape)}")
    seqlen_k = int(k.shape[1])
    if seqlen_k not in SUPPORTED_KV_TOKENS:
        raise ValueError(f"unsupported KV extent: {seqlen_k}")
    ...
    _persistent_tilequeue_d256_fwd[(PERSISTENT_PROGRAMS,)](
        q,
        k,
        v,
        out,
