...
v1.5.1-native-vl.10 在可搬移原生包内提供固定模型的图片、视频、混合媒体和多轮
...
> warmup，并通过严格配对门槛保留这份文本产品能力；v1.5.1-native-vl.5 新增
> 可选 thinking、受限工具调用进展，以及在推理期间仍能响应 health 与 shutdown
> 控制请求的串行 chat executor。v1.5.1-native-vl.6 修复默认 VL 请求在非流式
> 和 SSE 中的推理与正文分离，并保留固定运行库。
...
> v1.5.1-native-vl.10 支持工具结果中的图片和视频，保留调用历史与媒体访问校验；
> 附带媒体不会掩盖明确的工具错误。
...
先从[个人上游 v1.5.1-native-vl.10 Release](https://github.com/skyguan92/AIMA-AMD395-Qwen36-35B-Linux-Engine/releases/tag/v1.5.1-native-vl.10)
下载运行包与校验文件：
...
完整精度、每次测量值和组件哈希见
[补丁产品契约](native/product-contract-v1.5.1-native-vl.10.json)和发布后镜像的
哈希绑定证据；package-input qualification 同时随包保存为
`share/aima/qualification.json`。`.10` 压缩包必须在 AMD395 上隔离运行并完成一小时
单进程 text/image/video/mixed soak，随后回滚验证精确的 v1.5.1 基线包。`.4` 的
双机结果只按 fail-closed runtime diff 规则继承未变化的 portable userspace 与
provider closure，不会写成 `.10` 已在第二台机器实测。
新增文本检查点路径必须单独通过 cold/cache 生成、完整词表 logits、两种容量
配置及 19 格文本性能矩阵；历史内核结果不能替代这些新路径的实测。

运行 `make verify-evidence` 可校验最新镜像进仓库的汇总记录与全部原始证据树；
